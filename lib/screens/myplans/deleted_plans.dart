...
class _DeletedPlansPageState extends State<DeletedPlansPage> {
  ...
  Future<void> _loadDeletedPlans() async {
    setState(() {
      _isLoading = true;
    });
    ...
    try {
      ...
      _showSnackBar('Failed to load deleted plans: $e');
    }
  }
  ...
  Future<void> _restoreSelected() async {
    ...
    try {
      for (final plan in selectedPlans) {
        await PlanService.restorePlan(plan.id!);
      }

      if (!mounted) return;

      await _loadDeletedPlans();
  ...
  Future<void> _deleteSelectedPermanently() async {
    ...
    if (selectedPlans.any((plan) => !_isAdmin(plan))) {
      _showSnackBar('Only plan admins can permanently delete plans.');
      return;
    }
    ...
    final confirmed = await _showConfirmDialog(
      ...
      content: selectedPlans.length == 1
          ? 'Permanently delete "${selectedPlans.first.title}"? This cannot be undone.'
          : 'Permanently delete ${selectedPlans.length} plans? This cannot be undone.',
      confirmText: 'Delete',
    ...
    try {
      for (final plan in selectedPlans) {
        await PlanService.permanentDeletePlan(plan.id!);
      }
      ...
      _showSnackBar('${selectedPlans.length} plan(s) permanently deleted.');
    } catch (e) {
      _showSnackBar('Permanent delete failed: $e');
    } finally {
      if (mounted) {
        setState(() {
          _isProcessing = false;
  ...
  Future<void> _deleteSinglePermanently(Plan plan) async {
    if (plan.id == null) return;
    ...
    return Padding(
      ...
      child: Row(
        children: [
          ...
          const Spacer(),
          IconButton(
            onPressed: _isProcessing ? null : _loadDeletedPlans,
          ...
          const Expanded(
            child: Column(
              ...
              children: [
                ...
                SizedBox(height: 4),
                Text(
                  'Restore deleted plans or permanently delete admin-owned plans.',
                  style: TextStyle(
                    fontSize: 13,
                    color: Colors.black54,
                    height: 1.3,
