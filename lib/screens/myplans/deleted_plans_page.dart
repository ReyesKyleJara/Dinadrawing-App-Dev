...
class DeletedPlansPage extends StatefulWidget {
  ...
  final void Function(String sectionId, Plan plan) onArchive;
  final void Function(String sectionId, Plan plan) onDeletePermanently;
...
class _DeletedPlansPageState extends State<DeletedPlansPage> {
  ...
  List<_PlanEntry> get _deletedPlans => [
  ...
  void _selectAll() {
    setState(() {
      _selectedKeys
        ..clear()
        ..addAll(_deletedPlans.map(_keyFor));
    });
  }
  ...
  Future<void> _restoreSelected() async {
    final selectedEntries = _deletedPlans.where((e) => _selectedKeys.contains(_keyFor(e))).toList();
    ...
    if (confirmed == true && mounted) {
      for (final entry in selectedEntries) {
        widget.onArchive(entry.sectionId, entry.plan);
      }
      _clearSelection();
      ScaffoldMessenger.of(context).showSnackBar(
  ...
  Future<void> _deleteSelectedPermanently() async {
    ...
    if (confirmed == true && mounted) {
      for (final entry in selectedEntries) {
        widget.onDeletePermanently(entry.sectionId, entry.plan);
      }
      _clearSelection();
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(content: Text('${selectedEntries.length} plan(s) permanently deleted')),
      );
    }
  }
  ...
  Widget build(BuildContext context) {
    final deletedPlans = _deletedPlans;
      ...
      body: SafeArea(
        child: Column(
          children: [
                  ...
                  const Expanded(
                    child: Column(
                      ...
                      children: [
                        ...
                        SizedBox(height: 4),
                        Text(
                          'These plans will be permanently deleted after 30 days',
                          style: TextStyle(fontSize: 13, color: Colors.black54),
                        ),
                      ],
                    ),
                  ),
                  if (deletedPlans.isNotEmpty)
            ...
            Expanded(
              child: deletedPlans.isEmpty
                  ? const Center(child: Text('No deleted plans yet.'))
                  : ListView.separated(
                      padding: const EdgeInsets.fromLTRB(20, 0, 20, 24),
                      itemCount: deletedPlans.length,
                      separatorBuilder: (context, index) => const SizedBox(height: 12),
                      itemBuilder: (context, index) {
                        final entry = deletedPlans[index];
